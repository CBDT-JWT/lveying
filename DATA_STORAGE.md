...
- ✅ 发送新弹幕
- ✅ 清空弹幕列表
 - ℹ️ 每条弹幕记录发送者的IP地址（仅用于审核与日志，不会公开展示）
 - ℹ️ 每条弹幕记录发送者的IP地址（仅用于审核与日志，不会公开展示）。保存时会对 IPv6 映射的 IPv4 (`::ffff:1.2.3.4`) 进行规范化，存为 `1.2.3.4`。
 - ⚠️ 新增：管理员可封禁 IP，封禁列表保存在 `data/store.json` 的 `bannedIps` 字段中，被封禁的 IP 将无法发送新的弹幕。
 - ⚠️ 新增：管理员可封禁 IP，封禁列表保存在 `data/store.json` 的 `bannedIps` 字段中，被封禁的 IP 将无法发送新的弹幕。封禁一旦生效，会自动取消该 IP 发送者的历史“已审核”弹幕（即取消 public 展示）。
